 ...
 const LoginPage:React.FC = () => {
    ...
    const  onSubmit = async (event:any) => {
        ...
        try {
          ...
          if(firstName) {
            startSession(loginResponse.user, loginResponse.user.uid,firstName)  
          }
          navigate('/me')
        } catch (error: any) {
      ...
      <div className={styles.modalDiv}>
        <div className={styles.modal}>
          ...
          <form onSubmit={onSubmit}>
            ...
            />
            <p>У вас нету аккаунта?  
            <Link to='/registration' state={{ background: location }} ><span>
                    Зарегистрироваться
                  </span> </Link>
            </p>
            <Button  onClick={undefined} variant={"loginAuth"}>Войти</Button>
          </form>
        </div>
      </div>
    </div>
